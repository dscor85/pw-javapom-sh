...
mvn test -DsuiteXmlFile=src/test/resources/testrunners/testng_regressions.xml
```

**Important:** `SelfHealingReport.html` is only generated by `SelfHealingListener`, which is
registered in `testng_regressions.xml`. If you run a single class directly with
`mvn test -Dtest=LoginPageTest`, Maven Surefire builds its own throwaway suite for just that
class and **ignores `testng_regressions.xml` entirely** — the listener never runs, so no HTML
report gets written, even though `healed-locators.json` and `healing-attempts.json` still will
(those are written directly by `HealingEngine`, independent of any listener). Always run via
`-DsuiteXmlFile=...` (or however your suite is normally invoked) if you want the HTML report.

Report generation uses `ISuiteListener.onFinish(ISuite)`, which fires exactly once when the
entire `<suite>` finishes — not `ITestListener.onFinish(ITestContext)`, which would fire once
per `<test>` block. `testng_regressions.xml` has two `<test>` blocks (Home Page, Login Page);
with `ISuiteListener` the report is built once, after both have run, from every attempt logged
